...
// Nuevas funciones solicitadas basadas en el PDF
...
int main() {
    ...
    do {
        ...
        case 7:
            ...
            break;
        case 8:
            arbol = balancearArbol(arbol); // Se asigna el nuevo retorno para que la raíz se actualice
...
// --- IMPLEMENTACIÓN DE FUNCIONES ORIGINALES (SIN CAMBIOS) ---
...
/* Altura del Árbol (Pág. 8 del PDF):
   Es la longitud de la trayectoria más larga desde la raíz hasta una hoja.
*/

int obtenerAltura(Nodo* arbol) {
    if (arbol == nullptr) return -1; // Árbol vacío tiene altura -1
    else {
...
/* Clasificación de Nodos (Pág. 3 del PDF):
   Identifica la jerarquía de cada nodo en la estructura.
*/
...
void clasificarNodos(Nodo* arbol, Nodo* raizOriginal) {
    ...
    if (arbol == raizOriginal) {
        cout << "RAIZ";
    } else if (arbol->izq == nullptr && arbol->dere == nullptr) {
        cout << "HOJA";
    } else {
        cout << "NODO INTERNO";
    }
